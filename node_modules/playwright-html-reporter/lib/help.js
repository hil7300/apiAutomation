...
class Help {
    ...
    printStep(id, count, testStep, space) {
        ...
        if (testStep.location !== undefined) {
            title = `<p class="stepLine" style="padding-left: ${space}px">${icon + " " + testStep.title + " | <span class='fst-italic'>" + testStep.location.file.replace(path_1.default.resolve('./') + "/", "") + ":" + testStep.location.line}</span><label class="stepLineDuration">${this.convertMsToTime(testStep.duration)}</label></p>`;
    ...
    printScreenshotComparison(id, attachmentExpectedPath, attachmentActualPath, attachmentDiffPath) {
        let output = "";
    ...
    printScreenshot(id, attachmentPath) {
        let output = "";
    ...
    base64ToFile(base64data, filename) {
        ...
        fs_extra_1.default.outputFileSync(filename, buf);
    }
    compressFileContent(content, type) {
        if (this.execOptions.minifyAssets === true) {
            if (type === "js") {
                return uglify_js_1.default.minify(content).code;
            }
            else if (type === "css") {
                return (0, csso_1.minify)(content).css;
            }
            else {
                return content;
            }
